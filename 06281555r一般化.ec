(* ====================================================================
   BIKE KEM IND-CPA security proof (EasyCrypt, r2025.03).
   Reorganized in dependency order. Proof scripts are unchanged.
   ==================================================================== *)

(* ====================================================================
   1. CORE IMPORTS AND PARAMETERS
   ==================================================================== *)
require import AllCore IntDiv.

(* GENERALIZED: parameters are now abstract.
   Per-level instances (L1/L3/L5) clone this file and supply the concrete
   values together with proofs of the axioms below. *)
op r : int.
op w : int.
op t : int.
op l : int.
...
(* GENERALIZED: these were proved by smt for r=37,w=6.
   They are now axioms supplied per instance.
   - r_gt0, l_ge0 are LOAD-BEARING for the IND-CPA proof (ring clone, 2^l>0, |v|=l).
   - w_gt0, w_even, uneven_w_half (d odd), r_prime are BIKE design constraints,
     kept as axioms for documentation; the IND-CPA proof does NOT use them. *)
axiom r_gt0 : 0 < r.
axiom r_ge3 : 3 <= r.   (* GENERALIZED: alg2_ll uses ln(r-2); needs r-2>=1 *)
axiom l_ge0 : 0 <= l.
axiom w_gt0 : 0 < w.
axiom w_even : 2 %| w.
...
(* ====================================================================
   2. RING, TYPES, CONVERSIONS, DISTRIBUTIONS
   ==================================================================== *)
clone import PolyReduce.PolyReduceZp as PolyRing
  with op n = r, op p = 2
  proof *.
realize ge2_p. smt. qed.
realize gt0_n. smt(r_gt0). qed.   (* GENERALIZED: was smt *)
...
(* ====================================================================
   4. POLYNOMIAL INVERSION (Algorithm 2)
   ==================================================================== *)
module Algorithm2 = {
  proc invert(a : R) : R = {
    var i <- 1;
...
(* ====================================================================
   6. IND-CPA: ORACLE AND ADVERSARY INTERFACES
   ==================================================================== *)
op duniformK = duniformM.
...
(* ====================================================================
   7. HARDNESS ASSUMPTIONS ((2,1)-QC Codeword Finding / Syndrome Decoding)
   ==================================================================== *)
module type CF_Dist = { proc distinguish(h : R) : bool }.
module DQCCF (D : CF_Dist) = {
  proc main(b : bool) : bool = {
    var sk, sigma, hk, hu, h, r0;
    (sk, sigma, hk) <@ Bike.keyGen();
    hu <$ distribution_over_R;
...
(* ====================================================================
   11. AUXILIARY LEMMAS (point masses, enumerations, losslessness, vectors)
   ==================================================================== *)
...
proof.
  ...
  - rewrite get_dvector0E.
    + by rewrite /max /=; smt(l_ge0).   (* GENERALIZED: + l_ge0 *)
    have h2l : 0 < 2^l by smt(StdOrder.IntOrder.expr_gt0 l_ge0).   (* GENERALIZED: + l_ge0 *)
    smt().
...
lemma alg2_ll : islossless Algorithm2.invert.
    proof.
      ...
      by auto; smt(r_ge3).   (* GENERALIZED: was smt; ln(r-2) needs r>=3 *)
  qed.
...
proof.
  ...
  smt(l_ge0).   (* GENERALIZED: was smt(); needs 0 <= l for max 0 l = l *)
qed.
...
section Security.
  ...
  (* ---- Transparency / bridging equalities ---- *)
...
+ have hpos : 0%r < (2^l)%r by smt(StdOrder.IntOrder.expr_gt0 l_ge0).
...
qed.
  ...
  (* ---- Syndrome-decoding fingerprint bounds for BadL ---- *)
  lemma badL_fp_inv :
...
qed.
  ...
  (* ---- E_t membership + BadL syndrome equality ---- *)
  ...
  (* ---- Composition: bad-event upper bounds ---- *)
