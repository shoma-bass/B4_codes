(* ====================================================================
   ...
   Correspondence with the paper:
     ...
     main claim (axiom-free)      | Theorem2_concrete / Theorem2_concrete_spec
...
*** VERIFICATION STATUS (revision f, 2026-07-13) ***
     REVISION f VERIFIED: EasyCrypt r2025.03 / Alt-Ergo, exit 0
     (full reload from top, zero admits). Changes from revision e —
     closure of Evaluation Report §7.2 Code Major 1-5:
     ...
     Axiom inventory of this revision:
       hardness (in-section): qccf_hard, qcsd_ow_hard — used ONLY by the
         epsilon-form Theorem1/Theorem2; Theorem2_concrete(_spec) is
         axiom-free (see NOTE in section Security).
...
proof.
proc; auto => />.
smt(dHw_odd invp_odd dHw_supp parity_mul).
qed.
...
(* NOTE (Code Major 3): these axioms quantify over ALL modules, including
   computationally unbounded ones; an unbounded adversary can enumerate
   enumE_t and solve OW-qcsd, so eps_qcsd_ow cannot be read as a
   negligible bound under this quantification. They are kept only to
   state Theorem1/Theorem2 in the paper's epsilon notation. The
   axiom-free, reduction-relative statement is Theorem2_concrete /
   Theorem2_concrete_spec below, which is the artifact's main claim. *)
